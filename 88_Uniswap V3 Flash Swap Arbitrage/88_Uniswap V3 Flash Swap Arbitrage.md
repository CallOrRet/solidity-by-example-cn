# 88.Uniswap V3 Flash Swap Arbitrage
Uniswap V3 闪电交易套利示例
```solidity
...
contract UniswapV3FlashSwap {
    ...
    // WETH/USDC的例子
    // 卖高价的WETH -> 买低价的WETH -> 获得WETH利润
    // 输入WETH -> 输出USDC -> 输入USDC -> 输出WETH -> 获得WETH利润
    function flashSwap(
        address pool0,
        uint24 fee1,
        address tokenIn,
...
```
2. 执行以下命令以运行测试。
```soliditFORK_URL=https://eth-mainnet.g.alchemy.com/v2/613t3mfjTevdrCwDl28CVvuk6wSIxRPi
